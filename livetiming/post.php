<?php
	...
	if(isset($_POST["data"]) && $_POST["key"] == $secret) {
      ...
      $data64 = base64_decode($_POST["data"]);
      if ($data64 == false) {
        echo "Unable to decode base64!";
        die();
      }
			// $data = gzinflate(base64_decode($_POST["data"]));
			$data = gzinflate($data64);
			if($data == false) {
		...
		if($rebuild)
			rebuild_list($_POST["sessionid"],$_POST["subsessionid"]);
	}
			...
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					...
					if($path_parts["extension"] == "json") {
						...
						if(count($parts) == 4 && ($sid == 0 || $parts[0] == $sid) && ($ssid == 0 || $parts[1] == $ssid))
							$jsons[] = $parts;
					}
				}
			}
